...
class GooglePlaceService:
    ...
    async def search_text_places(self, request: PlaceTextSearchRequest) -> PlaceResponse:
        try:
            response = await self._client.post_text_search(request)
    ...
    async def search_nearby_places(self, request: PlaceNearbySearchRequest) -> PlaceResponse:
        try:
            response = await self._client.post_nearby_search(request)
        except httpx.HTTPStatusError as error:
            places_error = google_places_error_from_response(error.response)
            logger.error(
                "google_places.http_error code=%s upstream_status=%s",
    ...
    def _parse_places_response(self, response: httpx.Response) -> PlaceResponse:
        try:
